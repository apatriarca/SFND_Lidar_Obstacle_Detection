...
void render2DTree(Node<std::vector<float>> *node, pcl::visualization::PCLVisualizer::Ptr &viewer, Box window, int &iteration, uint depth = 0)
{
	if (node != NULL)
	{
...
int main()
{
	...
	auto tree = new KdTree<std::vector<float>, 2>;
	...
	//
	std::vector<std::vector<int>> clusters = euclideanCluster<std::vector<float>, 2>(points, *tree, 3.0f);
	//
